...
void get_params_json(dt_imageio_module_format_t *self, JsonBuilder *json_builder)
{
  ...
  dt_json_add_int_from_dt_conf(json_builder, "plugins/imageio/format/jpeg/quality");
  dt_json_add_int_from_dt_conf(json_builder, "plugins/imageio/format/jpeg/subsample");
...
int set_params_json(dt_imageio_module_format_t *self, JsonReader *json_reader)
{
  ...
  const uint32_t subsample = dt_json_get_int(json_reader, "subsample");
  json_reader_end_element(json_reader);

  dt_bauhaus_slider_set(g->quality, quality);
  dt_bauhaus_combobox_set(g->subsample, subsample);
  return 0;
}
