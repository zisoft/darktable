...
void get_params_json(dt_imageio_module_storage_t *self, JsonBuilder *json_builder)
{
  ...
  dt_json_add_string_from_dt_conf(json_builder, "plugins/imageio/storage/latex/file_directory");
  dt_json_add_int_from_dt_conf(json_builder, "plugins/imageio/storage/latex/title");
...
int set_params_json(dt_imageio_module_storage_t *self, JsonReader *json_reader)
{
  ...
  const gchar *file_directory = dt_json_get_string(json_reader, "file_directory");
  const gchar *title = dt_json_get_string(json_reader, "title");
  json_reader_end_element(json_reader);

  gtk_entry_set_text(GTK_ENTRY(g->entry), file_directory);
  dt_conf_set_string("plugins/imageio/storage/latex/file_directory", file_directory);
  gtk_entry_set_text(GTK_ENTRY(g->title_entry), title);
  dt_conf_set_string("plugins/imageio/storage/latex/title", title);
  return 0;
}
